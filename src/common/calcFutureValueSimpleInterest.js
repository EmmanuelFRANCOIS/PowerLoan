...
/**
 * Calculates the future value of an annuity for a simple interest loan.
 ...
 * calcFutureValueSimpleInterest(10000, 0.05, 10, "ordinary", 0);
 */
function calcFutureValueSimpleInterest(capital, interestRate, durationPeriods, annuityType = "ordinary", startingAmount = 0) {
  // Validate inputs
  ...
  if (r.eq(0)) {
    ...
    // Calculate future value based on annuity type
    const interestFactor = r.times(n);
    futureValue = P.plus(P.times(interestFactor));

    if (annuityType === "due") {
      // Increase future value by one period's interest if annuity is due
      futureValue = futureValue.plus(P.times(r));
    }
...
export default calcFutureValueSimpleInterest;
